...
class BaseDatos {
    ...
    public function __construct(){
        $this->HOSTNAME = "localhost"; // puede ser localhost  (direccion del servidor de la BD)
        $this->BASEDATOS = "bd_viajes"; // nombre de la base de datos 
        $this->USUARIO = "root";
        $this->CLAVE="";
        $this->RESULT=0;
        $this->QUERY="";
    ...
    public function Registro() {
        ...
        if ($this->RESULT){
            unset($this->ERROR);
            if($temp = mysqli_fetch_assoc($this->RESULT)){ // devuelve como un array asociativo
                $resp = $temp;
            }else{
